...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // validate request-line (menthod SP request-target SP HTTP-version CRLF
            char reqtarget[sizeof(line)- 15];
            memset(reqtarget, '\0', sizeof(reqtarget));
            strncpy(reqtarget, &line[4], strlen(&line[4]) - 11);
            ...
            // 505 HTTP version is not HTTP/1.1
            if (strcmp(strrchr(line, ' '), " HTTP/1.1\r\n") != 0)
            {
                error(505);
                continue;
            }
            // 501 absolute path doesn't contain '.'
            ...
            // isolate absolute-path)
            int size = 0;
            if (strrchr(reqtarget, '?') != NULL)
            {
                size = (strlen(strrchr(reqtarget, '?')));
            }
            
            // find query statment if given
            char questmk[size + 1];
            memset(questmk, '\0', sizeof(questmk));
            memcpy(questmk, strrchr(reqtarget, '?'), sizeof(questmk)- 1);
                        
            char absolute[(strlen(reqtarget)) - (strlen(questmk))];
            memset(absolute, '\0', sizeof(absolute));
            strncpy(absolute, reqtarget, sizeof(absolute) );
            
            
            // extract query from request-target
            char query[sizeof(questmk)];
            memset(query, '\0', sizeof(query));
            if (sizeof(questmk) > 1)
            {
                memcpy(query, &questmk[1], sizeof(query));
            }
            
                        
            // concatenate root and absolute-path
            char path[strlen(root) + strlen(reqtarget) + 1];
            memset(path, '\0', sizeof(path));
            strcpy(path, root);
            strcat(path, reqtarget);

            // 400 if absolute-path contains '?', '"'
            ...
            char extension[sizeof(strrchr(reqtarget, '.') + 1)];
            memset(extension, '\0', sizeof(extension));
            memcpy(extension, strrchr(reqtarget, '.') + 1, sizeof(extension));
            ...
            if (strcasecmp("php", extension) == 0)
            {
                ...
                char command[sizeof(format) + (sizeof(path) - 2) + (sizeof(query) - 2) + 1];
...
const char* lookup(const char* extension)
{
    // to lower and validate extension 
    int k = 0;
    while (extension[k])
    {
        putchar(tolower(extension[k]));
        k++;
    }   
    // return text/css if extention is css (tolower)
    ...
    // return image/gif if gif
     else if (strcmp(extension, "gif") == 0)
    {
        return "image/gif";
    }
    // return x-icon if ico
     if (strcmp(extension, "ico") == 0)
    {
        return "image/x-icon";
    }
    // return image/jpeg if jpg
     else if (strcmp(extension, "jpg") == 0)
    {
        return "image/jpeg";
    }
    // return text/javascript if js
     else if (strcmp(extension, "js") == 0)
    {
        return "text/javascript";
    }
    // return image/png if png
